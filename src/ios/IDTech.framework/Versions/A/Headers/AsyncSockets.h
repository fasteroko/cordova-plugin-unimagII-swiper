...
extern NSString *const AsyncSocketsException;
extern NSString *const AsyncSocketsErrorDomain;

enum AsyncSocketsError
{
	AsyncSocketsCFSocketError = kCFSocketError,	// From CFSocketError enum.
	AsyncSocketsNoError = 0,						// Never used.
	AsyncSocketsCanceledError,					// onSocketWillConnect: returned NO.
	AsyncSocketsConnectTimeoutError,
	AsyncSocketsReadMaxedOutError,               // Reached set maxLength without completing
	AsyncSocketsReadTimeoutError,
	AsyncSocketsWriteTimeoutError
};
typedef enum AsyncSocketsError AsyncSocketsError;

@protocol AsyncSocketsDelegate
@optional
...
**/
- (void)onSocket:(AsyncSockets *)sock willDisconnectWithError:(NSError *)err;
...
**/
- (void)onSocketDidDisconnect:(AsyncSockets *)sock;

/**
 * Called when a socket accepts a connection.  Another socket is spawned to handle it. The new socket will have
 * the same delegate and will call "onSocket:didConnectToHost:port:".
**/
- (void)onSocket:(AsyncSockets *)sock didAcceptNewSocket:(AsyncSockets *)newSocket;

/**
 * Called when a new socket is spawned to handle a connection.  This method should return the run-loop of the
 * thread on which the new socket and its delegate should operate. If omitted, [NSRunLoop currentRunLoop] is used.
**/
- (NSRunLoop *)onSocket:(AsyncSockets *)sock wantsRunLoopForNewSocket:(AsyncSockets *)newSocket;

/**
 * Called when a socket is about to connect. This method should return YES to continue, or NO to abort.
 * If aborted, will result in AsyncSocketsCanceledError.
 * 
...
**/
- (BOOL)onSocketWillConnect:(AsyncSockets *)sock;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)onSocket:(AsyncSockets *)sock didConnectToHost:(NSString *)host port:(UInt16)port;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)onSocket:(AsyncSockets *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)onSocket:(AsyncSockets *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)onSocket:(AsyncSockets *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)onSocket:(AsyncSockets *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)onSocket:(AsyncSockets *)sock
...
**/
- (void)onSocketDidSecure:(AsyncSockets *)sock;
...
@interface AsyncSockets : NSObject
{
...
// Once one of the accept or connect methods are called, the AsyncSockets instance is locked in
...
// When an incoming connection is accepted, AsyncSockets invokes several delegate methods.
 ...
 * When a connection comes in, the AsyncSockets instance will call the various delegate methods (see above).
 ...
 * 
 * Please note the recommended way of releasing an AsyncSockets instance (e.g. in a dealloc method)
 ...
 * 
 * If you pass a buffer, you must not alter it in any way while AsyncSockets is using it.
 ...
 * it is treated similarly to a timeout - the socket is closed with a AsyncSocketsReadMaxedOutError.
 ...
 * The performance can be much improved by allowing AsyncSockets to read larger chunks at a time and
...
/**
 * When you create an AsyncSockets, it is added to the runloop of the current thread.
...
/**
 * Returns the current run loop modes the AsyncSockets instance is operating in.
 * The default set of run loop modes is NSDefaultRunLoopMode.
**/
